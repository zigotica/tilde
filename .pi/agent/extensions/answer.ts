/**
 * Modified from: https://github.com/mitsuhiko/agent-stuff/blob/main/extensions/answer.ts
 * Changes by zigotica:
 * - remove all the model logic, when API key was detected
 *   it would try to use that, even if no credits available,
 *   which threw uninformative "Cancelled" messages.
 *   We now use current model instead.
 *
...
export default function(pi: ExtensionAPI) {
  const answerHandler = async (ctx: ExtensionContext) => {
    ...
    // Run extraction with loader UI (use current model)
    ...
    const extractionResult = await ctx.ui.custom<ExtractionResult | null>(
      (tui, theme, _kb, done) => {
        ...
        const doExtract = async () => {
          ...
          if (!auth.ok) {
            throw new Error(`No API key: ${auth.error}`);
          }
    ...
    if (extractionResult === null) {
      if (extractionError) {
        ...
        ctx.ui.notify("Cancelled: extraction null", "info");
      }
      return;
    }
    ...
    if (answersResult === null) {
      ctx.ui.notify("Cancelled: answers null", "info");
      return;
    }
